module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!PAYSTACK_SECRET) return res.status(500).json({ error: 'Paystack secret not set in Vercel Settings' });
    if (!amount || !momo_number || !name) return res.status(400).json({ error: 'Missing fields' });
    ...
    if (!d1.status) return res.status(400).json({ success:false, message:d1.message, details:d1 });
    ...
    if (!d2.status) return res.status(400).json({ success:false, message:d2.message, details:d2 });
